...
fn read_file_contents(
    ...
    match tree.path_value(path).into_resolved() {
        ...
        Ok(Some(TreeValue::Conflict(id))) => Err(BuiltinToolError::Unimplemented {
            item: "conflict",
            id: id.hex(),
        }),
        Err(merge) => Err(BuiltinToolError::Unimplemented {
            item: "conflict",
            id: merge.iter().flatten().map(|id| id.hex()).join(", "),
        }),
    }
}
...
pub fn apply_diff_builtin(
    ...
    let mut tree_builder =
        MergedTreeBuilder::new(left_tree.store().clone(), left_tree.id().clone());
    assert_eq!(
        changed_files.len(),
        files.len(),
        ...
        match selected {
            ...
            scm_record::SelectedContents::Binary {
                ...
                match value.into_resolved() {
                    ...
                    Err(merge) => {
                        tree_builder.set_or_remove(path, merge);
                    }
                }
            }
...
mod tests {
    ...
    fn test_edit_diff_builtin() {
        ...
        let left_tree = testutils::create_merged_tree(
            &test_repo.repo,
            &[
                (&unused_path, "unused\n"),
                (&unchanged, "unchanged\n"),
        ...
        let right_tree = testutils::create_merged_tree(
            &test_repo.repo,
            &[
                (&unused_path, "unused\n"),
                (&unchanged, "unchanged\n"),
    ...
    fn test_make_merge_sections() {
        ...
        fn file_id(tree_value: TreeValue) -> FileId {
            match tree_value {
                TreeValue::File { id, executable: _ } => id,
                other @ (TreeValue::Symlink(_)
                | TreeValue::Tree(_)
                | TreeValue::GitSubmodule(_)
                | TreeValue::Conflict(_)) => {
                    panic!("tree value should have been a FileId: {other:?}")
                }
            }
        }
        let merge = Merge::new(
            vec![base_tree.path_value(&path).map(file_id)],
            vec![
                left_tree.path_value(&path).map(file_id),
                right_tree.path_value(&path).map(file_id),
            ],
        );
